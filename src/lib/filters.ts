import { emitFiltersChanged } from './ui/rt-events';
import { isStringArray, readStoredJson, removeStoredValue, writeStoredJson } from './ui/storage';
...
export function loadFilters(): Set<string> {
  const active = readStoredJson(STORAGE_KEY, [], isStringArray);
...
export function saveFilters(active: Set<string>): void {
  if (active.size === 0) {
    ...
    writeStoredJson(STORAGE_KEY, [...active]);
  }
  emitFiltersChanged({ active: [...active] });
}
